...
fn handle_prompt_result(
    ...
    match result.outcome {
        ...
        PromptOutcome::Ok(_) => {
            ...
            // Did anything the agent said actually reach the channel? The
            // agent publishes its own reply through the CLI, so a turn can
            // finish cleanly having delivered nothing.
            if let (Some(text), Some(obs), Some(rest), PromptSource::Channel(channel)) = (
                result.agent.acp.take_turn_text(),
                observer.clone(),
                rest_client.cloned(),
                &result.source,
            ) {
                undelivered::spawn_check(
                    text,
